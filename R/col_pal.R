...
col_pal <- function(name = NULL,
  ...
  if (name %in% c("ggplot", "ggplot2", "hue", "hue_pal", "huepal")) {
    ...
    if (pal_select$type2 == "discrete") {
      ...
      pal_return <- paletteer::paletteer_d(pal_select$command, n = n, type = type, direction = 1)

    } else if (pal_select$type2 == "continuous") {
      if (is.null(n) || n == 0) {
        n <- 100
      }
      pal_return <- paletteer::paletteer_c(pal_select$command, n = n, direction = 1)
    }
  }
